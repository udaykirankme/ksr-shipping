...
import { ServicesApi, ServiceItem } from "@/lib/services-api";
...
export default function ServicesTab() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await ServicesApi.updateService(isEditing.id, formData);
        toast.success("Service updated");
      } else {
        await ServicesApi.createService({ ...formData, display_order: items.length });
  ...
  const handleMove = async (index: number, direction: 'up' | 'down') => {
    ...
    // Update display_order for all items to ensure consistency
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-700">Manage all services shown on the public website and used throughout the system.</p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-gray-700">No services found.</td>
                </tr>
              )}
            </tbody>
          </table>

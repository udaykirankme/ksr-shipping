...
import { ArrowLeft, Save, MapPin, Package, Clock, History, Loader2, Copy, ArchiveRestore, Trash2, Truck, Calendar, CheckCircle2, FileText, MoreVertical, Search, Edit2, AlertCircle, RefreshCw, Printer } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/shipments" className="p-2 hover:bg-gray-100 rounded-full transition-colors">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <PremiumDatePicker 
                    ...
                    onChange={(date) => setFormData((prev: any) => ({ ...prev, estimated_delivery: date.toISOString().split('T')[0] }))} 
                    disabled={isDelivered} 
                  />
                </div>
                <div>
              ...
              <form onSubmit={handleStatusUpdate} className="space-y-4">
                ...
                <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <PremiumDatePicker 
                      value={statusUpdate.occurred_at ? new Date(statusUpdate.occurred_at) : null}
                      onChange={(date) => {
                        const dateStr = date.toISOString().split('T')[0];
                        const timeStr = statusUpdate.occurred_at ? statusUpdate.occurred_at.split('T')[1] : '00:00';
                        setStatusUpdate((prev: any) => ({ ...prev, occurred_at: `${dateStr}T${timeStr}` }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <PremiumTimePicker 
                      value={statusUpdate.occurred_at ? statusUpdate.occurred_at.split('T')[1] : ''}
                      onChange={(time) => {
                        const dateStr = statusUpdate.occurred_at ? statusUpdate.occurred_at.split('T')[0] : new Date().toISOString().split('T')[0];
                        setStatusUpdate((prev: any) => ({ ...prev, occurred_at: `${dateStr}T${time}` }));
                      }}
                    />
                  </div>
                </div>
